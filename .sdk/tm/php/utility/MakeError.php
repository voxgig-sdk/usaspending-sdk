...
// Usaspending SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class UsaspendingMakeError
{
    public static function call(?UsaspendingContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UsaspendingContext([], null);
        }
        $op = $ctx->op ?? new UsaspendingOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UsaspendingResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UsaspendingError) ? $err->msg : (string)$err;
        $msg = "UsaspendingSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new UsaspendingError('', $msg, $ctx);
