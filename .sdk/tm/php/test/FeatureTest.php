...
class FtCtrl extends UsaspendingControl
{
...
class FtClient
{
    ...
    private UsaspendingUtility $utility;

    public function __construct(UsaspendingUtility $utility, array $options)
    {
    ...
    public function get_utility(): UsaspendingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UsaspendingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UsaspendingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UsaspendingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UsaspendingConfig::make_feature($name);
    ...
    private static function buildUrl(UsaspendingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UsaspendingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UsaspendingResponse($fetched);
        }
    ...
    private function fail(UsaspendingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UsaspendingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
