...
{
    private static function utility(): UsaspendingUtility
    {
        return new UsaspendingUtility();
    }
    ...
    private static function ctx(array $over = []): UsaspendingContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new UsaspendingContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new UsaspendingOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof UsaspendingError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new UsaspendingOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = UsaspendingMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new UsaspendingOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = UsaspendingMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new UsaspendingOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = UsaspendingMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UsaspendingSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = UsaspendingMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new UsaspendingResponse([]),
            'result' => new UsaspendingResult([]),
        ]);
        [, $err] = UsaspendingMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new UsaspendingSpec([]),
            'response' => null,
            'result' => new UsaspendingResult([]),
        ]);
        [, $err] = UsaspendingMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new UsaspendingSpec(['step' => 's']),
            'response' => new UsaspendingResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new UsaspendingResult([]),
        ]);
        [, $err] = UsaspendingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new UsaspendingSpec(['step' => 's']),
            'response' => new UsaspendingResponse(self::resp(200, ['v' => 1])),
            'result' => new UsaspendingResult([]),
        ]);
        [, $err] = UsaspendingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new UsaspendingSpec(['step' => 's']),
            'response' => new UsaspendingResponse(self::resp(200, ['v' => 2])),
            'result' => new UsaspendingResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        UsaspendingMakeResponse::call($ctx);
    ...
    {
        $preset = new UsaspendingResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new UsaspendingSpec([]),
            'response' => new UsaspendingResponse([]),
            'result' => new UsaspendingResult([]),
        ...
        [$got, $err] = UsaspendingMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new UsaspendingResult([])]);
        [, $err] = UsaspendingMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new UsaspendingSpec([]), 'result' => null]);
        [, $err] = UsaspendingMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UsaspendingSpec(['step' => 's']),
            'result' => new UsaspendingResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new UsaspendingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UsaspendingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UsaspendingSpec(['step' => 's']),
            'result' => new UsaspendingResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new UsaspendingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UsaspendingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UsaspendingResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new UsaspendingSpec([]),
            'result' => new UsaspendingResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = UsaspendingMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UsaspendingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new UsaspendingError('boom', 'boom');
        $utility->fetcher = function (UsaspendingContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UsaspendingSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = UsaspendingMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (UsaspendingContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = UsaspendingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(UsaspendingResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        UsaspendingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (UsaspendingContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UsaspendingSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = UsaspendingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new UsaspendingResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new UsaspendingSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = UsaspendingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = UsaspendingMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = UsaspendingMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new UsaspendingResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, UsaspendingDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new UsaspendingResult(['ok' => false])]);
        $this->expectException(UsaspendingError::class);
        UsaspendingDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new UsaspendingResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', UsaspendingMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        UsaspendingMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new UsaspendingBaseFeature();
        $b = new UsaspendingBaseFeature();
        UsaspendingFeatureAdd::call($ctx, $a);
        UsaspendingFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): UsaspendingBaseFeature
    {
        $f = new UsaspendingBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        UsaspendingFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): UsaspendingContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new UsaspendingSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UsaspendingPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        UsaspendingPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new UsaspendingResponse(['headers' => null]),
            'result' => new UsaspendingResult([]),
        ]);
        UsaspendingResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        UsaspendingResultBody::call($ctx);
