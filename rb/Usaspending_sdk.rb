...
require_relative 'Usaspending_types'
...
class UsaspendingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UsaspendingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UsaspendingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UsaspendingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UsaspendingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UsaspendingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UsaspendingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UsaspendingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = UsaspendingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UsaspendingError.new(
        "graphql_error", "UsaspendingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UsaspendingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
