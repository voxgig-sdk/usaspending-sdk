...
class SpendingEntity
  ...
  def make
    opts = @_entopts.dup
    SpendingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UsaspendingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Spending, Hash] the current Spending data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UsaspendingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Spending fields)
  def match_get
  ...
  #
  # @param reqmatch [SpendingListMatch, Hash, nil] match filter (any subset of
  #   Spending fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Spending>, Array] the matching Spending items; raises UsaspendingError on failure
