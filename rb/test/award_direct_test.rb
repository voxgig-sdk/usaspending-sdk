...
class AwardDirectTest < Minitest::Test
  def test_direct_list_award
    setup = award_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-award", setup[:live] ? "live" : "unit")
    if _should_skip
...
def award_direct_setup(mockres)
