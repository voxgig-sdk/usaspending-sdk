...
module UsaspendingOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if UsaspendingOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = UsaspendingOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| UsaspendingOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || UsaspendingOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = UsaspendingSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = UsaspendingError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { UsaspendingOmni.errify(err) }
