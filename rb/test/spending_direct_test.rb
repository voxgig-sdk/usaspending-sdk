...
class SpendingDirectTest < Minitest::Test
  def test_direct_list_spending
    setup = spending_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-spending", setup[:live] ? "live" : "unit")
    if _should_skip
...
def spending_direct_setup(mockres)
