...
module UsaspendingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UsaspendingResponse.new({})
    ...
    if fetch_err
      ...
      response = UsaspendingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UsaspendingResponse.new(fetched)
    else
