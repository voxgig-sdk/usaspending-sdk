...
//     (\`new UsaspendingSDK(...)\` / \`UsaspendingSDK.test(...)\` become
//     \`UsaspendingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"account":{"test01":{"id":"test01"}},"agency":{"test01":{"id":"test01"}},"award":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"spending":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UsaspendingSDK(...)\` and any
// \`UsaspendingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UsaspendingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UsaspendingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
