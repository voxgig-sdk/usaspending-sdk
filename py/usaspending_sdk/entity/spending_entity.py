...
from usaspending_sdk.usaspending_types import (
    Spending,
    SpendingListMatch,
)
...
class SpendingEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SpendingEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Spending]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Spending().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
