# Usaspending Python SDK

The Python SDK for the Usaspending API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Account(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Usaspending(None).load(
    {"id": "test01"}, None
)
