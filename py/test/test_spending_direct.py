...
class TestSpendingDirect:

    def test_should_direct_list_spending(self):
        setup = _spending_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-spending", "live" if setup["live"] else "unit")
        if _skip:
...
def _spending_direct_setup(mockres):
