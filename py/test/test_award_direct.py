...
class TestAwardDirect:

    def test_should_direct_list_award(self):
        setup = _award_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-award", "live" if setup["live"] else "unit")
        if _skip:
...
def _award_direct_setup(mockres):
