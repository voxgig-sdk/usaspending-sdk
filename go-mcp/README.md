...
[MCP](https://modelcontextprotocol.io) server exposing the Usaspending SDK as
two agent tools — `usaspending_list` and `usaspending_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/usaspending-mcp)
make build

# 2. Provide credentials via the environment
export USASPENDING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user usaspending \
  -- /absolute/path/to/usaspending-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./usaspending-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// usaspending_list: first page of records
{ "entity": "account" }
{ "entity": "account", "query": { } }

// usaspending_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/usaspending-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user usaspending \
     -- "$PWD"/dist/*/usaspending-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `usaspending_list` and `usaspending_load` tools now appear
   in new sessions. Ask the agent to *"list account using usaspending"*
   and it calls `usaspending_list` with `{"entity":"account"}`.
...
```sh
export USASPENDING_APIKEY=sk_live_xxx            # API key
export USASPENDING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `usaspending_list` tool
...
### Call the `usaspending_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `usaspending_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `usaspending_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `USASPENDING_APIKEY` | API key sent with every request. |
| `USASPENDING_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

account | agency | award | search | spending

### Smoke test via HTTP (raw JSON-RPC)

```sh
./usaspending-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"usaspending_load","arguments":{"entity":"account","query":{"id":1}}}}'
```
