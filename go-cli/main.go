// Command Usaspending-cli is a CLI + REPL for the Usaspending SDK,
...
const prompt = "usaspending"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "account agency award search spending"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      USASPENDING_APIKEY (api key), USASPENDING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: USASPENDING_APIKEY carries the API key and
	// USASPENDING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("USASPENDING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("USASPENDING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUsaspendingSDK(opts)
