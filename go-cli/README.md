...
boru-driven command-line client **and** interactive REPL for the Usaspending
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/usaspending-cli)
make build

# 2. See usage (words, entities, env vars)
./usaspending-cli --help

# 3. Provide credentials once, via the environment
export USASPENDING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./usaspending-cli list account
./usaspending-cli list agency

# 5. Override the API base URL for a single call
USASPENDING_BASE=https://api.example.com ./usaspending-cli list account

# 6. No arguments -> interactive REPL
./usaspending-cli
usaspending> list account
usaspending> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/usaspending-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/usaspending-cli list account
   ```
...
Run with no arguments to open a REPL (prompt `usaspending>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/usaspending-cli`. |
...
(`./usaspending-cli <expr>`) and interactively (the REPL), and expressions compose the
