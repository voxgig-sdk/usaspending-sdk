...
// Usaspending SDK primary utility test
//
...
//   UsaspendingOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with UsaspendingOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   UsaspendingOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = UsaspendingOmni::makeRunner(
                self::TEST_JSON_FILE, UsaspendingSDK::test(null, null));
    ...
    private static function client(): UsaspendingSDK
    {
    ...
    private static function make_test_ctx(
        UsaspendingSDK $client,
        UsaspendingUtility $utility,
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = UsaspendingOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = UsaspendingOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = UsaspendingOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new UsaspendingResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new UsaspendingBaseFeature();
    ...
    {
        $hook_client = UsaspendingSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends UsaspendingBaseFeature {
            public $hook_fn = null;
            public function TestHook(UsaspendingContext $ctx): void
            {
    ...
    {
        $init_client = UsaspendingSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends UsaspendingBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new UsaspendingSDK([
    ...
    {
        $blocked_client = new UsaspendingSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_request_basic(): void
    {
        ...
        $this->runsection('makeRequest', function (array $ctxmap) use ($client, $utility) {
            ...
            UsaspendingOmni::sync_ctx($ctxmap, $ctx);
            if ($err !== null) {
                throw $err;
            }
            return null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new UsaspendingSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = UsaspendingSDK::test(null, is_array($setup) ? $setup : null);
        $spec_utility = $spec_client->get_utility();
        $utility = self::client()->get_utility();

        $this->runsection('makeSpec', function (array $ctxmap) use ($spec_client, $spec_utility, $utility) {
            $ctx = UsaspendingOmni::ctx_from_map($ctxmap, $spec_client, $spec_utility);
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return UsaspendingOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_operator_basic(): void
    {
        $this->runsection('operator', function ($vin = null) {
            ...
            $op = new UsaspendingOperation($vin);
            return [
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = UsaspendingSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = UsaspendingOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof UsaspendingError)
