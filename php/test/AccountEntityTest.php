...
class AccountEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set USASPENDING_TEST_ACCOUNT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$account_ref01_list_result, $err] = $account_ref01_ent->list($account_ref01_match, null);
        $this->assertNull($err);
...
function account_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("USASPENDING_TEST_ACCOUNT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["USASPENDING_TEST_ACCOUNT_ENTID"]);
