...
local SpendingEntity = {}
SpendingEntity.__index = SpendingEntity


function SpendingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SpendingEntity)
  self._name = "spending"
  self._client = client
...
function SpendingEntity:get_name()
  return self._name
end


function SpendingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SpendingEntity.new(self._client, opts)
end
...
function SpendingEntity:mark_deleted()
  self._deleted = true
end


function SpendingEntity:deleted()
  return true == self._deleted
end


function SpendingEntity:data_set(args)
  if args ~= nil then
...
function SpendingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SpendingEntity:match_set(args)
  if args ~= nil then
...
function SpendingEntity:match_get()
...
function SpendingEntity:stream(action, args, callopts)
...
function SpendingEntity:_run_op(ctx, post_done)
  local utility = self._utility
