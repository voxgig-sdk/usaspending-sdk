...
local AwardEntity = {}
AwardEntity.__index = AwardEntity


function AwardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AwardEntity)
  self._name = "award"
  self._client = client
...
function AwardEntity:get_name()
  return self._name
end


function AwardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AwardEntity.new(self._client, opts)
end
...
function AwardEntity:mark_deleted()
  self._deleted = true
end


function AwardEntity:deleted()
  return true == self._deleted
end


function AwardEntity:data_set(args)
  if args ~= nil then
...
function AwardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AwardEntity:match_set(args)
  if args ~= nil then
...
function AwardEntity:match_get()
...
function AwardEntity:stream(action, args, callopts)
...
function AwardEntity:_run_op(ctx, post_done)
  local utility = self._utility
