...
local UsaspendingSDK = {}
UsaspendingSDK.__index = UsaspendingSDK
...
UsaspendingSDK._make_feature = _make_feature


function UsaspendingSDK.new(options)
  local self = setmetatable({}, UsaspendingSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function UsaspendingSDK:options_map()
...
function UsaspendingSDK:get_utility()
  return Utility.copy(self._utility)
end


function UsaspendingSDK:get_root_ctx()
  return self._rootctx
end


function UsaspendingSDK:prepare(fetchargs)
...
function UsaspendingSDK:direct(fetchargs)
...
-- by passing it.
function UsaspendingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function UsaspendingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "UsaspendingSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function UsaspendingSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = UsaspendingSDK.new(sdkopts)
  sdk.mode = "test"
