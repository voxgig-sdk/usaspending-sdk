...
The Lua SDK for the Usaspending API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Account(nil):list(nil, nil)
...
local result, err = client:Usaspending(nil):load(
  { id = "test01" }, nil
)
