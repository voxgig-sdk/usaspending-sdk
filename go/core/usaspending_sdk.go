...
func (sdk *UsaspendingSDK) OptionsMap() map[string]any {
...
func (sdk *UsaspendingSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *UsaspendingSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *UsaspendingSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *UsaspendingSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *UsaspendingSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *UsaspendingSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("UsaspendingSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *UsaspendingSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *UsaspendingSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("UsaspendingSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
