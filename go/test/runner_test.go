...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("USASPENDING_TEST_LIVE") == "TRUE" ||
		os.Getenv("USASPENDING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("USASPENDING_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.UsaspendingSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.UsaspendingError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.UsaspendingSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.UsaspendingError{Msg: msg, Code: code}
}
